...
export async function getPostBlocks(pageId, from, slice) {
  return getContentPage(pageId, from, slice)
}
export async function getPostPreviewBlocks(pageId, from, slice) {
  return getPage(pageId, from, slice)
}
...
export async function getGlobalData({
  ...
  try {
    ...
    if (shouldPreserveStaticCacheOnError()) {
      throw error
    }
  }
...
function shouldPreserveStaticCacheOnError() {
    ...
    allPages: [
      {
        ...
        summary: '访问文档获取帮助 → https://xiaolu.love',
        status: 'Published',
        type: 'Post',
        slug: 'oops',
        publishDay: '2024-11-13',
        pageCoverThumbnail: BLOG.HOME_BANNER_IMAGE || DEFAULT_SITE_IMAGE,
        date: {
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (!value) {
      console.warn('[Notion] skip page because block is missing', id)
      continue
    }
    if (value.parent_id && collectionId && value.parent_id !== collectionId) {
      console.warn('[Notion] skip page outside current collection', id)
      continue
    }
    ...
    try {
      ...
      console.warn('[Notion] skip page because parsing failed', id, error)
    }
  }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || 'ShowlookFans BLOG'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '这是一个由ShowlookFans生成的站点'
  const defaultPageCover =
    NOTION_CONFIG?.HOME_BANNER_IMAGE || DEFAULT_SITE_IMAGE
