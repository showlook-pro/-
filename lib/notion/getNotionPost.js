...
import { DEFAULT_CONTENT_PAGE_OPTIONS, getPage } from './getPostBlocks'
import { checkStrIsNotionId, checkStrIsUuid } from '@/lib/utils'
import md5 from 'js-md5'
import notionAPI from './getNotionAPI'
import { normalizeShowPagePropertiesMode } from './pageProperties'
...
export async function getPost(pageId, allPages = []) {
  ...
  if (!postInfo?.properties) {
    try {
      const rawRecordMap = normalizeRecordMap(await notionAPI.getPage(pageId))
  ...
  const showPagePropertiesMode = normalizeShowPagePropertiesMode(
    mappedProperties.show_page_properties
  )
  ...
  const fallbackTitle = normalizeString(
    safeGetTextContent(sourcePostInfo?.properties?.title)
  )
  const title =
    normalizeString(mappedProperties.title) ||
    fallbackTitle ||
    scopeParent?.title ||
    null
  const summary =
    normalizeString(mappedProperties.summary) || scopeParent?.summary || ''
  ...
  return {
    ...
    title,
    status: 'Published',
    show_page_properties_mode: showPagePropertiesMode,
    password,
...
function extractRawProperties(postInfo, blockMap) {
  ...
  Object.entries(pageProperties).forEach(([propertyId, value]) => {
    ...
    properties[propertyName] = safeGetTextContent(value)
  })
  return properties
}

function safeGetTextContent(value) {
  if (!value) {
    return ''
  }
