import {
  hasResolvedPlaceCoordinates,
  normalizePlaceCoordinates
} from '@/lib/notion/place'
...
export const validateFormFieldValue = (field, value) => {
  ...
  if (field?.type === 'place' && !hasResolvedPlaceCoordinates(normalizedValue)) {
    return '请先点击按钮获取当前位置。'
  }
...
export const serializeFormValue = (schema, value) => {
  ...
  switch (schema?.type) {
    ...
    case 'place': {
      const placeCoordinates = normalizePlaceCoordinates(normalizedValue)
