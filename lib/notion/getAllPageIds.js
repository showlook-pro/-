...
function collectIdsFromQueryView(pageIds, seen, queryView) {
  ...
  appendIds(pageIds, seen, queryView?.results?.blockIds)
...
function getCollectionViewIds(collectionView, viewIds) {
  if (Array.isArray(viewIds) && viewIds.length > 0) {
    return viewIds
  }

  return Object.keys(collectionView || {})
}
...
export default function getAllPageIds(
  ...
  if (preferredViewId) {
    ...
    collectIdsFromCollectionView(
      pageIds,
      seen,
      collectionView?.[preferredViewId]
    )
  }

  // 新版 Notion 中 page_sort 可能被截断，collection_query 可能包含补充记录。
  // 这里始终追加 query 中缺失的页面，保留已有顺序，同时避免漏读数据库记录。
  for (const queryView of Object.values(queryViews)) {
    collectIdsFromQueryView(pageIds, seen, queryView)
  }

  for (const viewId of getCollectionViewIds(collectionView, viewIds)) {
