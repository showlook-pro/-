...
const CONFIG_TABLE_PAGE_OPTIONS = {
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let pageRecordMap = await getPage(
    configPageId,
    'config-table',
    undefined,
    CONFIG_TABLE_PAGE_OPTIONS
  )
  ...
  try {
    // 将INLINE_CONFIG合并，@see https://xiaolu.love
