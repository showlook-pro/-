...
async function getNotionUser(userId) {
  const cacheKey = userId?.[1] || JSON.stringify(userId)
  if (!cacheKey) {
    return null
  }

  if (notionUserCache.has(cacheKey)) {
    return notionUserCache.get(cacheKey)
  }

  const res = await notionAPI.getUsers(userId)
  const resValue = res?.recordMapWithRoles?.notion_user?.[cacheKey]?.value
  const user = resValue
    ? {
        id: resValue.id,
  ...
  notionUserCache.set(cacheKey, user)
  return user
}
...
export default async function getPageProperties(
      ...
      switch (schema[key]?.type) {
        ...
        case 'person': {
          ...
          for (let i = 0; i < rawUsers.length; i++) {
            if (rawUsers[i][0][1]) {
              const user = await getNotionUser(rawUsers[i][0])
              if (user) {
                users.push(user)
              }
            }
  ...
  properties.unlock_scope = Array.isArray(properties.unlock_scope)
    ? properties.unlock_scope[0] || ''
    : properties.unlock_scope || ''
  properties.show_page_properties = normalizeShowPagePropertiesMode(
    properties.show_page_properties
  )
