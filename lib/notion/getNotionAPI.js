...
import { buildSyncRecordRequest, syncRecordValues } from './requestNotion'
...
async function callNotionWithRetry(
  ...
  try {
    ...
    if (retryAttempts <= 1 || isNotionRateLimitError(error)) {
      throw error
    }
...
function getFormLayoutPointers(recordMap) {
  ...
  for (const blockRecord of Object.values(recordMap?.block || {})) {
    ...
    if (layoutId && !recordMap?.layout?.[layoutId]) {
      formLayoutIds.add(layoutId)
    }
  }

  return Array.from(formLayoutIds)
}
...
async function hydrateFormBlocks(recordMap, pageOptions = {}) {
  ...
  try {
    const result = await getBlocksNormalized(
      formBlockIds,
      pageOptions?.ofetchOptions
    )
...
async function hydrateFormLayouts(recordMap) {
  normalizeRecordMap(recordMap)

  const formLayoutIds = getFormLayoutPointers(recordMap)
  if (!formLayoutIds.length) {
    return recordMap
  }

  try {
    const result = await syncRecordValues(
      formLayoutIds.map(id => buildSyncRecordRequest(id, 'layout')),
      {
        requireAuth: Boolean(BLOG.NOTION_TOKEN_V2)
      }
    )
    ...
    console.warn(
      '[NotionAPI] hydrate form layouts failed',
      { formLayoutIds },
      error?.message || error
    )
  }
...
async function getPageNormalized(pageId, pageOptions = {}) {
  const {
    hydrateCollections: shouldHydrateCollections = false,
    hydrateForms: shouldHydrateForms = false,
    ...notionPageOptions
  } = pageOptions || {}

  // 默认保持上游 NotionNext 的轻量 getPage；内嵌数据库/表单只在具体页面显式开启，避免全站数据请求被放大。
  const recordMap = await callNotion('getPage', pageId, notionPageOptions)
  ...
  if (shouldHydrateForms) {
    ...
    await hydrateFormLayouts(recordMap)
  }
