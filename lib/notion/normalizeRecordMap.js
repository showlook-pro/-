...
const normalizeCollectionTitleVisibility = recordMap => {
  if (recordMap?.block) {
    for (const entry of Object.values(recordMap.block)) {
      const block = entry?.value

      if (!COLLECTION_VIEW_BLOCK_TYPES.has(block?.type)) {
        continue
      }

      block.format = {
        ...(block.format || {}),
        hide_inline_collection_name: true
      }
    }
  }

  if (recordMap?.collection_view) {
    for (const entry of Object.values(recordMap.collection_view)) {
...
export function normalizeRecordMap(recordMap) {
  ...
  normalizeCollectionTitleVisibility(recordMap)
