import BLOG from '@/blog.config'
import {
  getDataFromCache,
  getOrSetDataWithCache,
  setDataToCache
} from '@/lib/cache/cache_manager'
import { optimizeRecordMapForClientRender } from './optimizeRecordMap'
import { normalizeRecordMap } from './normalizeRecordMap'
import { deepClone, delay } from '../utils'
import notionAPI from '@/lib/notion/getNotionAPI'
import { getBlockCollectionId } from 'notion-utils'
import { isNotionRateLimitError } from './errors'

const limit = createLimit(15)
const REQUEST_INTERVAL = 50
export const DEFAULT_CONTENT_PAGE_OPTIONS = {
  hydrateCollections: true,
  hydrateForms: true,
  collectionReducerLimit: 100
}
const CONTENT_PAGE_HYDRATION_OPTIONS = {
  collectionReducerLimit: 100
}

function createLimit(concurrency) {
  const queue = []
  let activeCount = 0

  const next = () => {
    if (activeCount >= concurrency || queue.length === 0) {
      return
    }

    activeCount++
    const { fn, resolve, reject } = queue.shift()

    Promise.resolve()
      .then(fn)
      .then(resolve, reject)
      .finally(() => {
        activeCount--
        next()
      })
  }

  return fn =>
    new Promise((resolve, reject) => {
      queue.push({ fn, resolve, reject })
      next()
    })
}

/**
 * 获取文章内容块
 * @param {*} id
 * @param {*} from
 * @param {*} slice
 * @returns
 */
export async function getPage(id, from = null, slice, pageOptions = {}) {
  const cacheKey = getPageCacheKey(id, pageOptions)
  return await getOrSetDataWithCache(
    cacheKey,
    async (id, slice, pageOptions) => {
      let pageBlock = await getDataFromCache(cacheKey)
      if (pageBlock) {
        // console.debug('[API<<--缓存]', `from:${from}`, cacheKey)
        return convertNotionBlocksToPost(
          id,
          normalizeRecordMap(pageBlock),
          slice
        )
      }

      // 抓取最新数据
      pageBlock = await limit(() => getPageWithRetry(id, from, 3, pageOptions))

      if (pageBlock) {
        await setDataToCache(cacheKey, pageBlock)
        return convertNotionBlocksToPost(
          id,
          normalizeRecordMap(pageBlock),
          slice
        )
      }
      return pageBlock
    },
    id,
    slice,
    pageOptions
  )
}

export async function getContentPage(id, from = null, slice) {
  const pageBlock = await getPage(id, from, slice)
  const hydrationOptions = getMissingContentHydrationOptions(pageBlock)

  if (!hydrationOptions) {
    return pageBlock
  }

  return await getPage(id, `${from || 'content'}-hydrate`, slice, {
    ...CONTENT_PAGE_HYDRATION_OPTIONS,
    ...hydrationOptions
  })
}

export function getPageCacheKey(id, pageOptions = {}) {
  const hydrationProfile = [
    pageOptions?.hydrateCollections ? 'collections' : null,
    pageOptions?.hydrateForms ? 'forms' : null
  ]
    .filter(Boolean)
    .join('_')

  return hydrationProfile
    ? `page_content_${id}_${hydrationProfile}`
    : `page_content_${id}`
}

function normalizeId(id) {
  return String(id || '')
    .replace(/-/g, '')
    .toLowerCase()
}

function getRecordValue(records, id) {
  if (!records || !id) {
    return null
  }

  return records[id]?.value || records[normalizeId(id)]?.value || null
}

function getCollectionQuery(recordMap, collectionId) {
  if (!recordMap?.collection_query || !collectionId) {
    return null
  }

  return (
    recordMap.collection_query[collectionId] ||
    recordMap.collection_query[normalizeId(collectionId)] ||
    null
  )
}

function hasCollectionQueryForView(recordMap, collectionId, viewId) {
  const collectionQuery = getCollectionQuery(recordMap, collectionId)
  if (!collectionQuery || !viewId) {
    return false
  }

  return Boolean(
    collectionQuery[viewId] || collectionQuery[normalizeId(viewId)]
  )
}

function getCollectionId(block, recordMap) {
  return (
    getBlockCollectionId(block, recordMap) ||
    block?.collection_id ||
    block?.format?.collection_pointer?.id ||
    null
  )
}

export function getMissingContentHydrationOptions(recordMap) {
  if (!recordMap?.block) {
    return null
  }

  let hydrateCollections = false
  let hydrateForms = false

  for (const blockRecord of Object.values(recordMap.block)) {
    const block = blockRecord?.value

    if (
      block?.type === 'collection_view' ||
      block?.type === 'collection_view_page'
    ) {
      const collectionId = getCollectionId(block, recordMap)
      const collection = getRecordValue(recordMap.collection, collectionId)
      const viewIds = Array.isArray(block.view_ids) ? block.view_ids : []

      for (const viewId of viewIds) {
        const view = getRecordValue(recordMap.collection_view, viewId)

        if (view?.type === 'form_editor') {
          const formBlockId = view?.format?.form_block_pointer?.id
          if (formBlockId && !getRecordValue(recordMap.block, formBlockId)) {
            hydrateForms = true
          }
          if (collectionId && !collection) {
            hydrateCollections = true
          }
          continue
        }

        if (
          collectionId &&
          (!collection ||
            !hasCollectionQueryForView(recordMap, collectionId, viewId))
        ) {
          hydrateCollections = true
        }
      }
    }

    if (block?.type === 'form') {
      const layoutId = block?.format?.form_layout_pointer?.id
      if (layoutId && !getRecordValue(recordMap.layout, layoutId)) {
        hydrateForms = true
      }
    }
  }

  if (!hydrateCollections && !hydrateForms) {
    return null
  }

  return {
    hydrateCollections,
    hydrateForms
  }
}

/**
 * 调用接口，失败会重试
 * @param {*} id
 * @param {*} retryAttempts
 */
export async function getPageWithRetry(
  id,
  from,
  retryAttempts = 3,
  pageOptions = {}
) {
  if (retryAttempts && retryAttempts > 0) {
    console.log(
      '[API-->>请求]',
      `from:${from}`,
      `id:${id}`,
      retryAttempts < 3 ? `剩余重试次数:${retryAttempts}` : ''
    )
    try {
      const start = new Date().getTime()
      const pageData = await notionAPI.getPage(id, pageOptions)
      const end = new Date().getTime()
      console.log('[API<<--响应]', `耗时:${end - start}ms - from:${from}`)
      return normalizeRecordMap(pageData)
    } catch (e) {
      console.warn('[API<<--异常]:', e)
      const cacheKey = getPageCacheKey(id, pageOptions)
      const pageBlock = await getDataFromCache(cacheKey, true)
      if (pageBlock) {
        console.warn('[API<<--回退缓存]', `from:${from}`, `id:${id}`)
        return pageBlock
      }

      if (isNotionRateLimitError(e)) {
        throw e
      }

      await delay(1000)
      return await getPageWithRetry(id, from, retryAttempts - 1, pageOptions)
    }
  } else {
    console.error('[请求失败]:', `from:${from}`, `id:${id}`)
    return null
  }
}

/**
 * Notion页面BLOCK格式化处理
 * 1.删除冗余字段
 * 2.比如文件、视频、音频、url格式化
 * 3.代码块等元素兼容
 * @param {*} id 页面ID
 * @param {*} blockMap 页面元素
 * @param {*} slice 截取数量
 * @returns
 */
function convertNotionBlocksToPost(id, blockMap, slice) {
  const clonePageBlock = deepClone(blockMap)
  let count = 0
  const blocksToProcess = Object.keys(clonePageBlock?.block || {})

  // 循环遍历文档的每个block
  for (let i = 0; i < blocksToProcess.length; i++) {
    const blockId = blocksToProcess[i]
    const b = clonePageBlock?.block[blockId]

    if (slice && slice > 0 && count > slice) {
      delete clonePageBlock?.block[blockId]
      continue
    }

    // 当BlockId等于PageId时移除
    if (b?.value?.id === id) {
      // 此block含有敏感信息
      delete b?.value?.properties
      continue
    }

    count++

    // === 【新增】强制修复非法 URL ===
    sanitizeBlockUrls(b?.value)

    if (b?.value?.type === 'sync_block' && b?.value?.children) {
      const childBlocks = b.value.children
      // 移除同步块
      delete clonePageBlock.block[blockId]
      // 用子块替代同步块
      childBlocks.forEach((childBlock, index) => {
        const newBlockId = `${blockId}_child_${index}`
        clonePageBlock.block[newBlockId] = childBlock
        blocksToProcess.splice(i + index + 1, 0, newBlockId)
      })
      // 重新处理新加入的子块
      i--
      continue
    }

    // 处理 c++、c#、汇编等语言名字映射
    if (b?.value?.type === 'code') {
      if (b?.value?.properties?.language?.[0][0] === 'C++') {
        b.value.properties.language[0][0] = 'cpp'
      }
      if (b?.value?.properties?.language?.[0][0] === 'C#') {
        b.value.properties.language[0][0] = 'csharp'
      }
      if (b?.value?.properties?.language?.[0][0] === 'Assembly') {
        b.value.properties.language[0][0] = 'asm6502'
      }
    }

    // 如果是文件，或嵌入式PDF，需要重新加密签名
    if (
      ['file', 'pdf', 'video', 'audio'].includes(b?.value?.type) &&
      b?.value?.properties?.source?.[0][0] &&
      (b?.value?.properties?.source?.[0][0].indexOf('attachment') === 0 ||
        b?.value?.properties?.source?.[0][0].indexOf('amazonaws.com') > 0)
    ) {
      const oldUrl = b?.value?.properties?.source?.[0][0]
      const newUrl = `https://notion.so/signed/${encodeURIComponent(oldUrl)}?table=block&id=${b?.value?.id}`
      b.value.properties.source[0][0] = newUrl
    }
  }

  optimizeRecordMapForClientRender(clonePageBlock, id)

  // 去掉不用的字段
  if (id === BLOG.NOTION_PAGE_ID) {
    return clonePageBlock
  }
  return clonePageBlock
}

/**
 * 根据[]ids，批量抓取blocks
 * 在获取数据库文章列表时，超过一定数量的block会被丢弃，因此根据pageId批量抓取block
 * @param {*} ids
 * @param {*} batchSize
 * @returns
 */
export const fetchInBatches = async (ids, batchSize = 100) => {
  // 如果 ids 不是数组，则将其转换为数组
  if (!Array.isArray(ids)) {
    ids = [ids]
  }

  let fetchedBlocks = {}
  if (ids.length === 0) {
    return fetchedBlocks
  }

  console.log('[Batch] START total ids:', ids.length)

  for (let i = 0; i < ids.length; i += batchSize) {
    const batch = ids.slice(i, i + batchSize)
    console.log(`\n[Batch] processing ${i} ~ ${i + batch.length}`)

    try {
      const blocks = await limit(async () => {
        await delay(REQUEST_INTERVAL)

        console.log('[API-->>批量请求]', batch.length)
        const start = Date.now()
        const pageChunk = await notionAPI.getBlocks(batch)
        const end = Date.now()
        const result = pageChunk?.recordMap?.block || {}

        console.log(
          `[API<<--批量响应] size:${batch.length} 耗时:${end - start}ms blocks:${Object.keys(result).length}`
        )

        return result
      })

      fetchedBlocks = {
        ...fetchedBlocks,
        ...blocks
      }
    } catch (err) {
      console.warn('[Batch API异常]', err.message)
    }
  }
  return fetchedBlocks
}

/**
 * 强制修复 block 中所有可能的非法 URL 字段
 * @param {Object} blockValue - block.value
 */
function sanitizeBlockUrls(blockValue) {
  if (!blockValue || typeof blockValue !== 'object') return

  const fixUrl = url => {
    if (typeof url !== 'string') return url

    if (url.startsWith('/')) {
      return url
    }

    // 修复 http:xxx → http://xxx
    if (url.startsWith('http:') && !url.startsWith('http://')) {
      url = 'http://' + url.slice(5)
    } else if (url.startsWith('https:') && !url.startsWith('https://')) {
      url = 'https://' + url.slice(6)
    }

    // 再次验证是否合法，否则替换为占位图
    try {
      new URL(url)
      return url
    } catch {
      console.warn('[Sanitize URL] Invalid URL replaced:', url)
      return 'https://via.placeholder.com/1x1?text=Invalid+Image'
    }
  }

  // 1. 处理 properties.source（用于 image, embed, bookmark, file, pdf 等）
  if (
    blockValue.properties?.source?.[0]?.[0] &&
    typeof blockValue.properties.source[0][0] === 'string'
  ) {
    blockValue.properties.source[0][0] = fixUrl(
      blockValue.properties.source[0][0]
    )
  }

  // 2. 处理 file.url（用于 file block）
  if (blockValue.file?.url && typeof blockValue.file.url === 'string') {
    blockValue.file.url = fixUrl(blockValue.file.url)
  }

  // 3. 处理 format.page_cover（页面封面）
  if (
    blockValue.format?.page_cover &&
    typeof blockValue.format.page_cover === 'string'
  ) {
    blockValue.format.page_cover = fixUrl(blockValue.format.page_cover)
  }

  // 4. 处理其他可能的 URL 字段（可选扩展）
  // 例如：video、audio 的 source 可能也走 properties.source，已覆盖
}
